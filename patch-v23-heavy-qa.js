...
// Portais traseiros v1.14: mesmo X/Z, checados por pavimento com listas separadas.
...
const masterClearances=bed&&ward?{
  wardrobeToBed:+Math.max(ward.min.x-bed.max.x,bed.min.x-ward.max.x,ward.min.z-bed.max.z,bed.min.z-ward.max.z,0).toFixed(2),
...
const pathBox=bounds('CAMINHO-LATERAL-V15');
const trees=[];scene.traverse(o=>{if(o.userData?.category==='Árvore frutífera')trees.push(o);});
const treeIssues=[];
for(const t of trees){
  const p=t.getWorldPosition(new THREE.Vector3()),wallDist=Math.min(4.94-Math.abs(p.x),12.44-Math.abs(p.z));
  if(wallDist>.55)treeIssues.push(`${t.userData.id||'árvore'} longe do muro (${wallDist.toFixed(2)} m)`);
  if(pathBox&&p.x>pathBox.min.x-.35&&p.x<pathBox.max.x+.35&&p.z>pathBox.min.z-.35&&p.z<pathBox.max.z+.35)treeIssues.push(`${t.userData.id||'árvore'} muito próxima do caminho`);
  if(p.x>3.45&&p.z>-2.2&&p.z<3.6)treeIssues.push(`${t.userData.id||'árvore'} na faixa da escada inferior`);
}
...
const qa={
  ...
  access:{social:!!byId('ACESSO-TRASEIRO-SOCIAL-V24'),private:!!byId('ACESSO-TRASEIRO-INTIMO-V24'),audit:window.__CASA_AUDIT_V24__||null},
  nav:window.__CASA_NAV_V20__||null,
  audits:{v20:window.__CASA_AUDIT_V20__||null,v21:window.__CASA_AUDIT_V21__||null,v22:window.__CASA_AUDIT_V22__||null,v24:window.__CASA_AUDIT_V24__||null},
  pass:duplicates.length===0&&invalid.length===0&&criticalOverlaps.length===0&&socialBlockers.length===0&&privateBlockers.length===0&&doorBlockers.length===0&&treeIssues.length===0&&outside.length===0&&!!byId('ACESSO-TRASEIRO-SOCIAL-V24')&&!!byId('ACESSO-TRASEIRO-INTIMO-V24')
};
window.__CASA_AUDIT_V23__=qa;
console.info('[Casa Contreras] HEAVY QA v1.15',qa);

let panel=document.getElementById('qa-v23');if(panel)panel.remove();
panel=document.createElement('div');panel.id='qa-v23';panel.style.cssText='position:fixed;right:12px;top:12px;z-index:18;width:min(440px,calc(100vw - 24px));max-height:calc(100vh - 24px);overflow:auto;background:rgba(8,13,11,.94);color:#eef5f0;border:1px solid rgba(255,255,255,.15);border-radius:12px;padding:12px;font:12px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;display:none;pointer-events:none;white-space:pre-wrap;box-shadow:0 14px 40px rgba(0,0,0,.35)';document.body.appendChild(panel);
let visible=false,lastUpdate=0;
function fmtArr(name,a){return `${name}: ${a.length?`FAIL (${a.length})\n  - ${a.join('\n  - ')}`:'OK'}`;}
function updatePanel(now){if(!visible||now-lastUpdate<220)return;lastUpdate=now;const nav=window.__CASA_NAV_V20__||{};const access=window.__CASA_AUDIT_V24__||{};panel.textContent=[
`CASA CONTRERAS — QA v1.15`,
...
``,
`ACESSOS TRASEIROS`,
`social: ${byId('ACESSO-TRASEIRO-SOCIAL-V24')?'OK':'FALTA'} | íntimo: ${byId('ACESSO-TRASEIRO-INTIMO-V24')?'OK':'FALTA'}`,
`v1.14 pass: ${access.pass===true?'SIM':access.pass===false?'NÃO':'?'}`,
``,
`FOLGAS`,
...
``,
fmtArr('overlaps críticos',criticalOverlaps),fmtArr('bloqueios social',socialBlockers),fmtArr('bloqueios íntimo',privateBlockers),fmtArr('bloqueios de portas',doorBlockers),fmtArr('árvores',treeIssues),fmtArr('fora do lote',outside),fmtArr('IDs duplicados',duplicates),fmtArr('transforms inválidos',invalid),``,`K = ocultar painel`
].join('\n');}
...
const top=document.getElementById('topbar');if(top)top.innerHTML=`<b>CASA CONTRERAS — v1.15 HEAVY QA / ACCESS</b><br><span class="muted">acessos traseiros corretos • navegação contínua • layout funcional • realismo leve • K abre auditoria em tempo real<br>7,076 × 6,058 m • referência humana 1,65 m • QA de overlaps/passagens/perímetro ativo</span>`;
