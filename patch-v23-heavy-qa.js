...
const treeIssues=[];
for(const t of trees){const p=t.getWorldPosition(new THREE.Vector3()),wallDist=Math.min(4.94-Math.abs(p.x),12.44-Math.abs(p.z));if(wallDist>.55)treeIssues.push(`${t.userData.id||'árvore'} longe do muro (${wallDist.toFixed(2)} m)`);if(pathBox&&p.x>pathBox.min.x-.35&&p.x<pathBox.max.x+.35&&p.z>pathBox.min.z-.35&&p.z<pathBox.max.z+.35)treeIssues.push(`${t.userData.id||'árvore'} muito próxima do caminho`);if(p.x>3.45&&p.z>-2.2&&p.z<3.6)treeIssues.push(`${t.userData.id||'árvore'} na faixa da escada inferior`);}
