...
// Cor e iluminação: realista, barata e estável.
...
for(const m of mats){
  ...
  const {r,g,b}=m.color,metalness=m.metalness??0,rough=m.roughness??1;
  ...
  }else if(m.isMeshStandardMaterial&&rough>.92&&metalness<.1){
    m.bumpMap=bump.fabric;m.bumpScale=.020;materialStats.fabric++;
  }
  m.needsUpdate=true;
}

// Água com profundidade e movimento visual mínimo.
...
for(const [id,mat] of[['LAGO-NATURAL-V15',waterMat],['LAGO-PEIXES-V15',fishWaterMat]]){
  const g=byId(id);if(!g)continue;let waterMesh=null;
...
const ribData=[];
function pushRib(w,h,d,x,y,z){ribData.push({w,h,d,x,y,z});}
function ribPanel(y,z,side){const x0=side<0?-halfW+.10:halfW-.78,x1=side<0?-halfW+.78:halfW-.10;for(let x=x0;x<=x1+.001;x+=.105)pushRib(.018,2.36,.024,x,y+1.19,z);}
...
for(const y of[LEVEL.social,LEVEL.private])for(const z of[front+.05,back-.05]){pushRib(HOUSE.w-.18,.035,.035,0,y+.08,z);pushRib(HOUSE.w-.18,.035,.035,0,y+2.42,z);}
if(ribData.length){const inst=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),ribMat,ribData.length),d=new THREE.Object3D();ribData.forEach((q,i)=>{d.position.set(q.x,q.y,q.z);d.scale.set(q.w,q.h,q.d);d.rotation.set(0,0,0);d.updateMatrix();inst.setMatrixAt(i,d.matrix);});inst.instanceMatrix.needsUpdate=true;ribs.add(inst);}

// Microdetalhes interiores sem colisão.
...
const bladeGeo=new THREE.ConeGeometry(.018,.15,3,1,false),bladeMatA=new THREE.MeshLambertMaterial({color:0x4e7443,side:THREE.DoubleSide}),bladeMatB=new THREE.MeshLambertMaterial({color:0x668452,side:THREE.DoubleSide});
const rnd=seeded(25162026),bladeData=[[],[]];
for(let i=0;i<520;i++){
  const x=-4.75+rnd()*9.50,z=-12.10+rnd()*24.20;
  const inHouse=x>-4.15&&x<4.55&&z>front-1.8&&z<back+1.8,inParking=x>.85&&z<-6.45,inLeftPath=x>-4.72&&x<-3.72&&z>-11.3&&z<11.6;
  const nearPond=((x+2.25)**2/2.0+(z+7.35)**2/4.3<1.25)||((x-1.72)**2/1.1+(z+5.30)**2/2.0<1.25);
  if(inHouse||inParking||inLeftPath||nearPond){i--;continue;}bladeData[i%2].push({x,z,s:.75+rnd()*.70,r:rnd()*Math.PI});
}
for(let v=0;v<2;v++){const arr=bladeData[v],inst=new THREE.InstancedMesh(bladeGeo,v?bladeMatB:bladeMatA,arr.length),d=new THREE.Object3D();arr.forEach((q,i)=>{d.position.set(q.x,.075*q.s,q.z);d.rotation.set((rnd()-.5)*.14,q.r,(rnd()-.5)*.14);d.scale.set(1,q.s,1);d.updateMatrix();inst.setMatrixAt(i,d.matrix);});inst.instanceMatrix.needsUpdate=true;inst.castShadow=false;grassRoot.add(inst);}

// Folhagem fina em 2 draw calls.
...
const leafGeo=new THREE.IcosahedronGeometry(1,0),leafMatA=new THREE.MeshLambertMaterial({color:0x3f703f}),leafMatB=new THREE.MeshLambertMaterial({color:0x527d46}),leafData=[[],[]];
...
const audit={version:'v1.16-realism-upgrade',dimensionsPreserved:{lot:[10,25],house:[7.076,6.058],human:1.65},renderer:{toneMapping:'ACESFilmic',exposure:renderer.toneMappingExposure,dprCap:.94,dynamicShadows:false},lights:{hemisphere:hemis.length,directional:dirs.length},materials:materialStats,waterBodiesUpgraded:upgradedWater,containerRibs:ribData.length,containerRibDrawCalls:ribData.length?1:0,interiorMicroDetails:detailCount,grassInstances:bladeData[0].length+bladeData[1].length,fruitTrees:trees.length,fineLeafInstances:leafData[0].length+leafData[1].length,geometryLayoutChanged:false,pass:upgradedWater>=2&&ribData.length>20&&detailCount>8&&trees.length>0&&(bladeData[0].length+bladeData[1].length)>=400};
window.__CASA_AUDIT_V25__=audit;
console.info('[Casa Contreras] AUDIT v1.16 REALISM',audit);
const top=document.getElementById('topbar');if(top)top.innerHTML=`<b>CASA CONTRERAS — v1.16 REALISM UPGRADE</b><br><span class="muted">microtexturas PBR leves • água com clearcoat/ondas • vidro refinado • corrugação instanciada • grama 3D • folhagem fina • rodapés/cozinha detalhados • iluminação ACES recalibrada<br>7,076 × 6,058 m • referência humana 1,65 m • nenhuma geometria arquitetônica foi redimensionada • K mantém QA técnico</span>`;
