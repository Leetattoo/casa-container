...
const MAT={
 ...
 wood:new THREE.MeshStandardMaterial({color:0xa36b40,roughness:.62}),
 glass:new THREE.MeshStandardMaterial({color:0xa7c6c8,transparent:true,opacity:.22,roughness:.12,depthWrite:false,side:THREE.DoubleSide}),
 wall:new THREE.MeshStandardMaterial({color:0xe8e2d8,roughness:.88}),
 chrome:new THREE.MeshStandardMaterial({color:0xb9c0bd,roughness:.16,metalness:.88}),
 green:new THREE.MeshLambertMaterial({color:0x3f7544}),
 soil:new THREE.MeshLambertMaterial({color:0x503522})
};
...
// ---------------------------------------------------------------------------
// 1) ACESSO SOCIAL REAL — porta lateral alinhada ao topo da escada térreo/social.
// ---------------------------------------------------------------------------
...
if(panelEnd>panelStart){
  box({w:.03,h:2.30,d:panelEnd-panelStart,x:sideX,y:LEVEL.social+1.16,z:(panelStart+panelEnd)/2,mat:MAT.glass,parent:socialSide});
  for(let z=panelStart+.18;z<panelEnd;z+=.82)box({w:.045,h:2.38,d:.045,x:sideX+.012,y:LEVEL.social+1.19,z,mat:MAT.steel,parent:socialSide});
}
...
// folha de correr estacionada ao lado do vão: visualmente existe porta, mas passagem fica aberta.
...
// ---------------------------------------------------------------------------
// 2) ACESSO ÍNTIMO REAL — abre a esquadria traseira no patamar superior.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 3) QUARTO DOS FILHOS — libera porta e elimina toque bancada/armário/treliche.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 4) MICRODETALHES LEVES — cozinha e interiores menos "bloco".
// ---------------------------------------------------------------------------
const detail=group('MICRODETALHES-V18','Microdetalhes arquitetônicos leves','Acabamento');
// puxadores/linhas na cozinha posterior
...
// plantas interiores: adicionam escala/vida sem assets pesados
for(const [x,y,z,s] of[[2.62,LEVEL.social,-1.82,.78],[-3.00,LEVEL.social,1.30,.64],[2.82,LEVEL.private,-1.55,.60]]){
  box({w:.34*s,h:.34*s,d:.34*s,x,y:y+.17*s,z,mat:MAT.wood,parent:detail,rounded:true});
  const dirt=mesh(new THREE.CylinderGeometry(.13*s,.15*s,.08*s,12),MAT.soil,detail);dirt.position.set(x,y+.34*s,z);
  for(let k=0;k<5;k++){const leaf=mesh(SPH,MAT.green,detail);leaf.scale.set(.08*s,.22*s,.06*s);leaf.position.set(x+(k-2)*.055*s,y+.52*s+(k%2)*.10*s,z+((k%2)?-.04:.04)*s);leaf.rotation.z=(k-2)*.22;}
}

// ---------------------------------------------------------------------------
// 5) QA DE ESPAÇO E ACESSO — falhas críticas ficam expostas no runtime.
// ---------------------------------------------------------------------------
const pairs=[
 ['ILHA-V15','JANTAR-V15'],['ILHA-V15','BANHEIRO-SOCIAL-V15'],['JANTAR-V15','SOFA-V15'],
...
const audit={
 version:'v1.8-access-space',
 dimensionsLocked:[HOUSE.w,HOUSE.d],
 externalStairs:!!byId('ESCADAS-EXTERNAS-V16'),
 socialEntry:{present:!!socialDoor,clearWidth:.94,alignedToExternalStair:true},
 privateEntry:{present:!!privateDoor,clearWidth:.96,alignedToRearBalcony:true},
 privateFurnitureRepositioned:!!kidsDesk&&!!kidsWard&&!!bunk,
 criticalOverlaps:overlapsFound,
 previousSpatialAudit:window.__CASA_AUDIT_V17__||null,
 pass:!!socialDoor&&!!privateDoor&&overlapsFound.length===0
};
